...
const Category: React.FC<CategoryProps> = ({ categoryId, title }) => {
    ...
    useEffect(() => {
        const fetchCategoryAnime = async () => {
            try {
                const categoryRes = await fetch(`http://localhost:8080/api/anime/category/get-category/${categoryId}`);
                const categoryData = await categoryRes.json();

                const animePromises = categoryData.animeIds.map(async (animeId: string) => {
                    const response = await fetch(`http://localhost:8080/api/anime/get-anime/${animeId}`);
