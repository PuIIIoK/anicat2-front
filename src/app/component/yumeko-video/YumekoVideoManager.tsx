...
const YumekoVideoManager: React.FC<Props> = ({ animeId, onClose }) => {
    ...
    const handleDrop = async (e: React.DragEvent<HTMLDivElement>) => {
        ...
        if (videoFiles.length === 0) {
            alert('Пожалуйста, выберите MP4 или MKV файл(ы)');
            return;
        }
        ...
        const autoNumbering = confirm(
            `Вы загружаете ${videoFiles.length} файлов. ` +
        ...
        if (autoNumbering) {
            ...
            console.log(`📤 Начата параллельная загрузка ${videoFiles.length} файлов`);
        } else {
            // Ручной режим - загружаем первый файл в форму
            setVideoFile(videoFiles[0]);
            alert(`Выбран файл ${videoFiles[0].name}. Укажите номер эпизода вручную.`);
        }
    };
                                        ...
                                        {!useLocalFile ? (
                                            ...
                                            >
                                                ...
                                                <label htmlFor="video-file-input" className={`file-upload-label ${videoFile ? 'has-file' : ''}`}>
                                                    <Upload size={20} />
                                                    <span>{videoFile ? videoFile.name : isDragging ? 'Отпустите файл(ы) здесь' : 'Выберите или перетащите MP4/MKV файл(ы)'}</span>
                                                </label>
                                                {videoFile && (
                                                    <button
                                                        type="button"
